...
def main(data_dir, model_name, num, lr, epochs, batch_size = 16, download_data = False, save_results=False):
    ...
    # Access the last classifier or prediction head
    #last_layer = model.classifier

    # Example: Modify the last layer for transfer learning
    #num_ftrs = last_layer[-1].in_features  # Example: Get the number of input features
    #last_layer[-1] = torch.nn.Linear(num_ftrs, 20)  # Replace last layer for a different number of output classes
...
#    std=[0.229, 0.224, 0.225]
    ...
    #originial size 300, 300 
    transformations = transforms.Compose([transforms.Resize((300, 300)),
    #transformations = transforms.Compose([transforms.Resize((224, 224)), #300
#                                      transforms.RandomChoice([
#                                              transforms.CenterCrop(224), #300
#                                              transforms.RandomResizedCrop(224, scale=(0.80, 1.0)), #300
#                                              ]),                                      
                                      transforms.RandomChoice([
    ...
    transformations_valid = transforms.Compose([transforms.Resize(300), # 330
    ...
    model.load_state_dict(torch.load(weights_file_path))
    transformations_test = transforms.Compose([transforms.Resize(330), #330 
                                          transforms.FiveCrop(330),    #330
...
if __name__ == '__main__':
    main('./data/', "resnet34", num=1, lr = [1.5e-4, 5e-2], epochs = 25, batch_size=5, download_data=False, save_results=True)
